...
describe('Search Tool', () => {
  describe('Schema Validation', () => {
    ...
    it('should reject query shorter than 3 characters', () => {
      ...
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('query must be at least 3 characters');
      }
    });
    ...
    it('should reject query longer than 200 characters', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('query must be at most 200 characters');
      }
    });
    ...
    it('should reject limit above 50', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('limit must be at most 50');
      }
    });
  ...
  describe('Search Functionality', () => {
    it('should return results for "MCP setup" query', async () => {
      ...
      const mcpSetupResults = result.results.filter((r) =>
        r.path.includes('02-mcp-setup-guide.md')
      );
